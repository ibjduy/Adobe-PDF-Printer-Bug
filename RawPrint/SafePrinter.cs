...
namespace RawPrint
{
    internal class SafePrinter : SafeHandleZeroOrMinusOneIsInvalid 
    {
        ...
        public static SafePrinter OpenPrinter(string printerName, ref PRINTER_DEFAULTS defaults, ref PRINTER_OPTIONS options)
        {
            IntPtr hPrinter;

            if (NativeMethods.OpenPrinter2W(printerName, out hPrinter, ref defaults, ref options) == 0)
            {
                throw new Win32Exception();
            }
